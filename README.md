...
│   ├── load_to_mysql.ipynb
│   └── schema.png                # ERD
├── Markdown/                     # Rendered notebook exports (GitHub-readable)
├── Images/                       # Chart images referenced by Markdown/
...
The notebooks are the main narrative (step-by-step, with reasoning in markdown); the `Script/` `.py` files reproduce the same cleaning and modeling pipeline without the walkthrough, for anyone who just wants to regenerate the processed data and model directly. See `Script/README.md` for details.
...
Nadiya — Ironhack Data Analytics Bootcamp, Final Capstone Project.
